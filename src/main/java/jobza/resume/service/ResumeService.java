...
public class ResumeService {
    ...
    public void uploadResume(MultipartFile resume) {
        ...
        if (fileExtension.equals("pdf")) {
            String storeImageName = uploadFileToS3(resume);
            log.info("storeImageName: " + storeImageName);
            String s3Url = amazonS3Client.getUrl(bucket, storeImageName).toString();
    ...
    public String uploadFileToS3(MultipartFile multipartFile) {
        String storeImageName = createStoreImageName(".pdf");
        InputStream inputStream;
        try {
            inputStream = multipartFile.getInputStream();
            ObjectMetadata metadata = new ObjectMetadata();
            metadata.setContentLength(multipartFile.getSize());
            amazonS3Client.putObject(new PutObjectRequest(bucket, storeImageName, inputStream, metadata));
            log.info("s3 업로드 완료");
        } catch (IOException e) {
